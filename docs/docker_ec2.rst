...
.. code-block:: yaml
    ...
    webapp:
        build: webapp:
        command: /usr/bin/python3.7 manage.py runserver 0.0.0.0:8001 # dev setting
        # command: gunicorn -b 0.0.0.0:8001 wsgi:application # production setting
        volumes:
...
Both are set to run on port `0.0.0.0:8001`, which is where the Docker daemon will discover it. You can now run `sudo docker-compose -f production.yml up` and browse to `localhost:8001` to see your application running.
